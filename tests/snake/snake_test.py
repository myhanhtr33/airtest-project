...
def get_match_boxes(poco):
    result = poco.invoke("get_macth_boxes")
    return result['matchBoxes']
...
def find_optimal_move(block_by, snake_map, match_boxes, block_by2):
    ...
    for box_index, box in enumerate(match_boxes):
        if not is_open_box(box):
            continue
        color= box_color(box)
        # print(f"type of color: {type(color).__name__}, color: {color}")
        # print(f"type of snake_map[3]['color']: {type(snake_map['snake3']['color']).__name__}, snake_map[3]['color']: {snake_map['snake3']['color']}")
        candidates=[
            sid for sid in snake_map.keys()
            if snake_map[sid]['color']== color
        ]
        if not candidates:
            raise RuntimeError(f"No snake found for box color {color}")
        print(f"box {box_index} with color {color} has candidates: {candidates}")
